...
$out = "Product Id,Winery,Name,Year,Size,Type,Appellation,Alcohol,UPC,TTB,CA Wholesale, NY Wholesale,In Store,Retail Price,Retail Sale\n";
$counter = 0;
while($row = $mydb->getAssoc($result)) {
  ...
  if ($row["alcohol"] == '0')
    ...
    $alcohol = $row["alcohol"];
  $out .=$row["product_id"].",".$row["winery_name"].",".$row["vintage_name"].",".$year.",".$row["size"].",".$row["type_name"].",".$row["appellation"].",".$alcohol.",".$row["upc"].",".$row["ttb"].",".$row["ca_wholesale_price"].",".$row["ny_wholesale_price"].",".$in_store.",".$row["price"].",".$row["sale"]."\n";
