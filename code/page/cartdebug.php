...
if ((isset($_POST["submit_update"])) || (isset($_POST["apply_discount"]))) {
  ...
  if ($_SESSION["cart_total"] == 0) {
    ...
    $sql = "SELECT discount_id, type, discount_from, discount_to from order_discount where name = '".$discount_name."' ";
    ...
    //if ((isWineClubMember($_SESSION["userCakeUser"])) && (!checkWineClubCartQtyCompliance($_SESSION["userCakeUser"],  $_SESSION["cart_qty"]))) {
    //$warnings = array();
      //$warnings[] = lang("WINECLUB_CART_WARNING");
      //$_SESSION["userCakeWarnings"] = $warnings;
    //}
    $successes = array();
    $successes[] = lang("CART_UPDATE_SUCCESS");
    $_SESSION["userCakeSuccesses"] = $successes;
...
if ($_SESSION["cart"]) {
  ...
  if ($discount_id > 0) {
    ...
    if ($row) {
      ...
      $smarty->assign('discount_min_bottles',$discount_min_bottles); // debug
    }
    $bottle_quantity = 0;
    foreach($cart as $k => $cartitem) {
    ...
    if ((($discount_percent > 0) || ($discount_value > 0)) &&
	(checkDiscountWindow($discount_from, $discount_to))) {
  ...
  foreach($cart as $k => $cartitem) {
	...
	$cart_qty = $cart_qty + $cartitem["quantity"];
	$name = BDW_GIFTCARD_PRODUCTID." for ".$cartitem["recipient_name"];
	$url = "/giftcard/";
	$title = $name;
	...
	if ($discount_type == BDW_DISCOUNT_TYPE_COUNTRY) {
	  $country = getWineryCountry($cartitem["winery_id"]);
	}
	if ($discount_percent > 0) { // only discount percent implemented
	  if (($discount_min_bottles <= $discount_bottle_quantity) && 
	      (checkDiscountTargetNew($discount_type, $discount_type_value, $discount_type_value_string, $cartitem["vint_id"], $cartitem["winery_id"], $cartitem["appellation"], $country)) &&
