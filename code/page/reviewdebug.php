...
if (TRUE) {
  ...
  if (!empty($_POST)) {  //coming from billship 
	...
	if ($cartitem["deleted"] == '0') {
	  if ($cartitem["type"] == BDW_LINE_ITEM_GIFTCARD) {
	    $ident = createGiftCard($cartitem["created_date"], $cartitem["purchaser_email"], $cartitem["purchaser_name"],$cartitem["recipient_email"],$cartitem["recipient_name"],$cartitem["price"],$cartitem["message"], $cartitem["type"], $cartitem["delivery_date"]);
	    $quantity = 1;
  ...
  if ($order["shipping_method"] == "Contact Me") { // the old way
	...
	//mail($toc, $subject, $message, $reply);
      ...
      $x_cancel_url = "https://www.bluedanubewine.com/reviewdebug";    
	...
	$smarty->assign('x_adult_signature_saving_description', $x_adult_signature_saving_description);
      ...
      //if ($debug_flag == TRUE) { 	$mydb->update("store_order",$log_values, " WHERE o_id = '".$order["o_id"]."'");       }
    }
  }
}
