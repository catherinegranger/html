...
function getNew($vintage, $last_3_months) {
  if ($vintage["vcreated"] > $last_3_months)
    return(1);
  return(0);
}
...
function getProduct($vintage, $last_3_months) {
  ...
  if ($vintage["item_type"] > 0) { 
    ...
    return(array('vint_id' => $vintage["vint_id"],
		 ...
		 'country' => $vintage["full_name"],
		 'country_webname' => $vintage["country_webname"],
		 'stock' => $vintage["stock"],
		 'low_inventory' => $vintage["low_inventory"],
		 'grup' => $vintage["grup"],
		 'bottles' => $vintage["bottles"],
...
function getSearchSql($criteria, $last_3_months, $countries, $varietals, $search_keywords, $wine_only, $limit) {
  ...
  if ($criteria == 'samplers') {
    ...
    $sql .= " WHERE v.deleted = '0' AND v.online_store = '1' AND v.stock = '1' AND v.wy_id = wy.wy_id AND v.type_id = t.type_id AND wy.c_id = c.c_id";
    $sql .= " AND grup = '1'";
    if ($order_by_sql) 
      $sql .= $order_by_sql;
    else 
  ...
  elseif ($criteria == 'new') {
    ...
    $sql .= " FROM vintage AS v, winery AS wy, country_code AS c, type AS t";
    $sql .= " WHERE v.deleted = '0' AND v.online_store = '1' AND v.stock = '1' AND v.wy_id = wy.wy_id AND v.type_id = t.type_id AND wy.c_id = c.c_id";
    $sql .= " AND v.created > '".$last_3_months."'";
    $sql .= $wine_only_sql;
    if ($order_by_sql) 
      $sql .= $order_by_sql;
    else 
  ...
  elseif (($criteria == 'others') || ($criteria == 'books')) {
    ...
    $sql .= " WHERE v.deleted = '0' AND v.online_store = '1' AND v.stock = '1' AND v.item_type > 0";
    if ($order_by_sql) 
      $sql .= $order_by_sql;
    else 
      $sql .= " ORDER BY v.name";
...
$wine_types[] = array('full_name' => 'New Arrivals', 'web_name' => 'new', 'url' => '/wines2/new/');
$wine_types[] = array('full_name' => 'Special Offers', 'web_name' => 'specials', 'url' => '/wines2/specials/');
$wine_types[] = array('full_name' => 'Organic', 'web_name' => 'organic', 'url' => '/wines2/organic/');
$wine_types[] = array('full_name' => 'Gifts & Samplers', 'web_name' => 'samplers', 'url' => '/wines2/samplers/');
$wine_types[] = array('full_name' => 'Gift Cards', 'web_name' => 'giftcard', 'url' => '/giftcard/');
$wine_types[] = array('full_name' => 'Books & Misc.', 'web_name' => 'books', 'url' => '/wines2/books/');
...
$wine_styles[] = array('full_name' => 'Reds', 'web_name' => 'red', 'url' => '/wines2/red/');
$wine_styles[] = array('full_name' => 'Whites', 'web_name' => 'white', 'url' => '/wines2/white/');
$wine_styles[] = array('full_name' => 'Rosé', 'web_name' => 'rose', 'url' => '/wines2/rose/');
$wine_styles[] = array('full_name' => 'Dessert', 'web_name' => 'dessert', 'url' => '/wines2/dessert/');
$wine_styles[] = array('full_name' => 'Sparkling', 'web_name' => 'sparkling', 'url' => '/wines2/sparkling/');
...
$smarty->assign('keyword',$two); // debug
...
if (isset($two)) {
  $_SESSION["redirectShop"] = "wines2/".$two."/";
}
