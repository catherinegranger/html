...
class NewUser {
  ...
  function __construct($username,$displayname,$password,$email)	{
  ...
  public function userCakeAddNewUser($role) {  // newer function with roles
    global $mysqli,$emailActivation,$websiteUrl,$db_table_prefix;
    //Prevent this function being called if there were construction errors
    if ($this->status) {
      //Construct a secure hash for the plain text password
	...
	/* Build the template - Optional, you can just use the sendMail function Instead to pass a message. */
	if (!$mail->newTemplateMsg("new-registration.txt",$hooks)) {
	  $this->mail_failure = true;
	}
	else {
	...
	$this->success = lang("ACCOUNT_REGISTRATION_COMPLETE_TYPE2");
      ...
      else { //Instant account activation
	$this->user_active = 1;
	$this->success = lang("ACCOUNT_REGISTRATION_COMPLETE_TYPE1");
      }	
      if (!$this->mail_failure) { //Insert the user into the database providing no errors have been found.
	$stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."users (
					user_name,
					display_name,
					password,
	...
	$stmt->close();
	if ($role > 1)
	  addPermission(array(1,$role), $this->user_id);
	else
	  addPermission(1, $this->user_id);
	return(TRUE);
      }
    }
    return(FALSE);
